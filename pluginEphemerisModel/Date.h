...
public:
	 ...
	 */
	static time_t interval(time_t a, time_t b) {
		a -= b;
	 ...
	 */
	inline static time_t getSystemTime() {
	 ...
	 */
	void unpause() {
		// catch the possible situation where the user unpauses without 
		// pausing first
		if( !paused )
			pause();
		paused = false;
